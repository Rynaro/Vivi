...
artefact arrives with a sibling `.envelope.json`, Vivi MUST NOT process the
...
Vivi's tool surface cannot run the SHA-256 gate itself (receiver Eidolons
have restricted or no Bash). The mechanical check therefore runs **at the
orchestrator**, once, before Vivi is dispatched:
...
`message_id`. Vivi then enforces the result using **only `Read`** — no Bash
required:
...
**Defense-in-depth (optional):** if Vivi's host happens to grant Bash with
...
## Inbound edges Vivi accepts

`to.eidolon` MUST equal `vivi`. `from.eidolon` MUST be a declared upstream
sender. Performative and `artifact.kind` are validated against the ECL inbound
contract `contracts/<from>-to-vivi.yaml` (source of truth); the table below
...
2. Print to stderr: `[vivi-verify-incoming] REFUSE: <FAILURE_CODE> from <from.eidolon>`.
